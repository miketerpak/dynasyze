 ...
 * TODO
 * 
 *  For AWS, wait for bucket to exist (finished? test)
 *
...
class Server {
    ...
    constructor({ whitelist = [], aliases = {}, subdirectory = "", store = {}, cache = {}, port = 3000 } = {}) {
        this.port = port
        this.aliases = aliases
        this.whitelist = whitelist
        this.subdirectory = (subdirectory + '/').replace('//','/')
    ...
    start(port) {
        ...
        app.get(this.subdirectory+'*', (req, res) => {
            let _this = this
            let parts = req.path.replace(this.subdirectory, '/').split('/').slice(1)
